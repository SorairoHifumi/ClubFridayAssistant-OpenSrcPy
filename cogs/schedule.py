...
async def getClassInfo(ctx,grade,class_number,period,dotw):
        ...
        try:   
            ...
            try:
                ...
                url = 'http://127.0.0.1'
            try:
                next_url = urls[grade][class_number][schedule[grade][class_number][dayoftoday][iperiod]]
            except:
                url = 'http://127.0.0.1'
...
```'''
        #value init done
            ...
            if (dotw > 5):
                embed = discord.Embed(title='Weekend!',description= "If you think I'm wrong, try using +cindiv. More info at +clistcmd",color=dotwColor['Weekend'])
            ...
            print(url)
            print(f'finalize {class_number} {type(class_number)} {dayoftoday}') # For troubleshooting
            embed = discord.Embed(title = f'{schedule[grade][class_number][dayoftoday][iperiod - 1]}',url= url ,color=color)
            ...
            if (dotw > 5):
                embed = discord.Embed(title='Weekend!',description= "If you think I'm wrong, try using <cindiv. More info at <help",color=dotwColor['Weekend'])
                embed.set_footer(text = f'{dayoftoday} {grade}/{class_number} #{period}')
                await ctx.send(embed = embed)
            else:
                embed = discord.Embed(title='Unavailable',description= "It's either you spell something wrong, I don't have it in my database.\n Or it could be that I have error in my database",color=dotwColor['Weekend'])
                embed.set_footer(text = f'{dayoftoday} {grade}/{class_number} #{period}')
                await ctx.send(embed = embed)
    ...
    async def call(self,ctx,dayofrequest,grade,class_number):
        ...
        try:
            for subject in schedule[grade][class_number][dayofrequest]:
                ...
                embed=discord.Embed(title=the_subject, color = dotwColor[dayofrequest],url = url)
                embed.add_field(name="Teacher", value=teacher_name, inline=False)
                await ctx.send(embed=embed)
        except:
            embed=discord.Embed(title="Cannot Fetch more...", description="There was an error while looking up the database", color=0xff3333)
            await ctx.send(embed=embed)
...
```'''
    ...
    async def cindiv(self,ctx,dayofreq,grade,class_number,period):
        try:
            ...
            embed.set_footer(text=f'Unaccepted Input :{dayofreq} 6/{class_number} {period}')
            await ctx.send(embed=embed)    
    @commands.command()
    async def hr(self,ctx,grade,class_number):
        ...
        if dotw > 5:
            ...
            dayoftoday = sourceDict['dotweek'][dotw - 1]
        try:
            embed = discord.Embed(title = f'Homeroom', description = f'Click the link to be redirected to Homeroom',url=urls[grade][class_number]['Homeroom'],color=dotwColor[dayoftoday])  
        except: 
